<?php
/**
 * UserAssociationTable class. Connects a user object with a model.
 *
...
/**
 * Class UserAssociationTable
 * @package IronBound\DB\Table\Association
 */
class UserAssociationTable extends BaseAssociationTable {
	...
	/**
	 * UserAssociationTable constructor.
	 *
	...
	public function __construct( Table $model_table, array $overrides = array() ) {
		...
		if ( ! empty( $overrides['slug'] ) ) {
			...
			$this->slug = $model_table->get_slug() . '-users';
		}

		if ( ! empty( $overrides['table_name'] ) ) {
			$this->table_name = $overrides['table_name'];
		} else {
			$this->table_name = str_replace( '-', '_', "{$model_table->get_slug()}_to_users" );
		}
	...
	public function get_columns() {
		return array(
			...
			$this->get_col_b() => new ForeignUser( $this->get_col_b() )
		);
	}
